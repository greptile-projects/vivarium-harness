...
import { armsForDisplay } from "../../arms.js";
...
export function LiveApp({
  ...
  // Give the last frame a moment to paint, then hand the terminal back. A
  // held run (the demo) skips this: the final frame is the thing being
  // inspected, so it stays until the human presses q.
  useEffect(() => {
    if (!model.finished || model.hold) return;
    const id = setTimeout(() => exit(), 400);
    return () => clearTimeout(id);
  }, [model.finished, model.hold]);
  ...
  const feedLines =
    selected === "log" ? model.log() : selected === "notes" ? model.notes() : null;
  ...
  // The run is over but the view is still up on purpose — say so in the footer,
  // undimmed, or the only cue that nothing more will move is the spinner
  // stopping.
  const held = model.finished && model.hold;
  ...
  return (
    <Box flexDirection="column" width={columns} height={rows - 1} paddingX={2} paddingTop={1}>
      ...
      <Box>
        <Text dimColor={!held} wrap="truncate-end">
