...
// `process.env`: Bun loads `.env`, so the harness process holds both repository
// URLs and both `<ARM>_GH_TOKEN`s. Forwarding all
...
export function cleanEnv(codexHome?: string): Record<string, string> {
