...
describe("run artifacts", () => {
  ...
  it("writes the run's own configuration without either arm's token", async () => {
      ...
      arms: [
        {
          name: "control",
          repo: "/tmp/control",
          ghToken: "ghp_control_live_token",
        },
        {
          name: "greptile",
          repo: "/tmp/greptile",
          reviewer: "greptile-apps[bot]",
          ghToken: "ghp_greptile_live_token",
        },
      ],
    ...
    // intends to publish.
    expect(written).not.toContain("ghp_control_live_token");
    expect(written).not.toContain("ghp_greptile_live_token");
  ...
  it("records what an attempt spent, including one that failed", async () => {
    ...
    const status = JSON.parse(
      await readFile(
        join(artifacts.directory, "control", "attempt-01", "status.json"),
        "utf8",
      ),
    );
    ...
    expect(manifest.arms.control.tokens).toBe(42_000);
  });
});
...
describe("redactArmConfig", () => {
  it("drops the token but keeps whether there was one", () => {
    expect(
      redactArmConfig({ name: "control", repo: "/r", ghToken: "ghp_x" }),
    ).toEqual({ name: "control", repo: "/r", ghTokenPresent: true });
    expect(redactArmConfig({ name: "control", repo: "/r" })).toEqual({
      name: "control",
      repo: "/r",
