...
async function makeConfig(): Promise<HarnessConfig> {
  ...
  return {
    ...
    arms: [
      {
        name: "komodo",
        repo: join(root, "komodo"),
        ghToken: "ghp_komodo_secret",
      },
      {
        name: "tuatara",
        repo: join(root, "tuatara"),
        reviewer: REVIEWER,
        ghToken: "ghp_tuatara_secret",
      },
    ],
...
describe("runHarness landing", () => {
  it("syncs, builds, answers the review, merges and records it", async () => {
    ...
    expect(manifest.arms.tuatara.tokens).toBe(5_000);
    expect(manifest.arms.komodo.tokens).toBe(1_000);
    ...
    expect(configJson).not.toContain("ghp_tuatara_secret");
    expect(configJson).not.toContain("ghp_komodo_secret");
    ...
    expect(land.commits[0].sha).toBe("commit-tuatara");
    ...
    expect(await readFile(land.diffFile, "utf8")).toContain("+tuatara");
    expect(JSON.stringify(manifest.arms.tuatara.landing)).not.toContain(
      "@@ -1 +1 @@",
    );
