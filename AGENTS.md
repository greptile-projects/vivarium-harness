...
- **`src/harness/state.ts`** — the climb's durable record, and the deliberate
  ...
  `climbLogPath` beside them, so the live feeds are filed by the same
  coordinates as the records they narrate rather than by a parallel scheme),
  and `readClimbState` reassembles the climb by
